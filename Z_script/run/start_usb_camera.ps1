# start_usb_camera.ps1 - 用「外接 USB 摄像头」启动智慧渔业系统（默认带传感器模拟数据）
# 依次启动：mediamtx + ffmpeg(USB摄像头→RTSP) + 传感器模拟器(datatran_test.py) + Flask(Web)，
# 按 Ctrl+C 退出时自动清理本次启动的所有子进程。
#
# 用法（在项目根下执行；脚本用 $PSScriptRoot 自动定位项目根，不依赖固定盘符）：
#   powershell -ExecutionPolicy Bypass -File .\Z_script\run\start_usb_camera.ps1             # 默认：带模拟数据
#   powershell -ExecutionPolicy Bypass -File .\Z_script\run\start_usb_camera.ps1 -NoSensor   # 去掉模拟数据
#   powershell -ExecutionPolicy Bypass -File .\Z_script\run\start_usb_camera.ps1 -VideoSize 800x600 -Fps 15
#
# 摄像头：本机外接 USB 摄像头通常是 "USB Video Device"（yuyv422，上限 1080p@5 / 720p@10 较优），
#         设备名为参数默认值并非写死；默认名找不到时脚本会自动枚举可用摄像头（唯一自动选/多个交互选）。
# 注意：本文件必须以 UTF-8 with BOM 保存（否则 Windows PowerShell 5.1 中文乱码解析失败）。

param(
    [string]$DeviceName = "USB Video Device",          # 外接 USB 摄像头设备名（可 -DeviceName 覆盖）
    [string]$VideoSize  = "1280x720",                  # 采集分辨率（受设备档位限制，设太高自动回落）
    [int]$Fps           = 10,                          # 采集帧率
...
try {
    ...
    # ---------- 2. ffmpeg 推 USB 摄像头 → RTSP（yuyv422 原始帧必须实时重编码 H.264） ----------
    ...
    Start-Sleep -Seconds 2
    if ($ffmpeg.HasExited) {
        Write-Host "  [警告] ffmpeg 启动即退出：摄像头可能被占用，或设备名/分辨率不对（720p@10 是较优档）。" -ForegroundColor Yellow
    }
