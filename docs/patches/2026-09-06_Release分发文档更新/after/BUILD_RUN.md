...
> **推荐（Clone 即用）**：FFmpeg / mediamtx 均为第三方二进制、不入 git，clone 后 `tools/` 不全属正常。
> 本仓库 **Releases** 提供打包好的 `tools_*.zip`（内含 `mediamtx.exe` + `ffmpeg`）——下载后解压到项目根即得 `tools/`，无需单独安装。
> 维护者可用 `Z_script\pack_tools.ps1` 重新打包（产物在 `dist/tools_*.zip`，gitignore）。
> 以下为手动放置 / 安装方式（无现成 zip 时的兜底）：

- **FFmpeg**（二选一，推荐自带版）：
  - 自带版：把 `ffmpeg.exe` 放到 `tools\ffmpeg\bin\`。`Z_script` 脚本会自动优先使用、找不到再回退系统 PATH（文件不入 git）。
  - 系统版：下载解压，把 `bin` 加入系统 PATH。
- **mediamtx**：从 https://github.com/bluenviron/mediamtx/releases 下载解压到 `tools/mediamtx/`（`mediamtx.exe` 不入 git）。首次运行会自动生成 `auto.crt`/`auto.key`（本地 TLS 凭证，不入库、无需分发）。
